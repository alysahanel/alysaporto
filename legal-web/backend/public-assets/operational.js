...
const REMIND_MONTHS = 2; 
const toBool01 = v => (v===true||v===1||v==='1'||String(v).toLowerCase()==='true')?1:0;
...
function makeKey(row) {
  ...
  return [doc_name, category, institution, exp].join('|');
}

router.get('/export', async (_req, res) => {
  try {
    const [rows] = await pool.query(`
      SELECT doc_name, type AS category, institution,
             DATE_FORMAT(start_date,'%Y-%m-%d')   AS start_date,
             DATE_FORMAT(expired_date,'%Y-%m-%d') AS expired_date,
             DATE_FORMAT(remind_date,'%Y-%m-%d')  AS remind_date,
             inactive
      FROM operational_permits ORDER BY id ASC
    `);
    const records = rows.map((r,i)=>({
      ...
      'TANGGAL KEDALUARSA': r.expired_date||'',
      'TANGGAL PENGINGAT (−2 bln)': r.remind_date||'',
      'STATUS': calcStatus(r),
    }));
    let csv = stringify(records, { header:true });
    csv = '\uFEFF' + csv;
    res.setHeader('Content-Type','text/csv; charset=utf-8');
    res.setHeader('Content-Disposition','attachment; filename="operational_permits.csv"');
...
router.get('/', async (_req,res)=>{
  const [rows] = await pool.query(`
    SELECT id, doc_name, type AS category, institution,
           DATE_FORMAT(start_date,'%Y-%m-%d')   AS start_date,
           DATE_FORMAT(expired_date,'%Y-%m-%d') AS expired_date,
           DATE_FORMAT(remind_date,'%Y-%m-%d')  AS remind_date,
           inactive
...
router.get('/:id', async (req,res)=>{
  const [rows] = await pool.query(`
    SELECT id, doc_name, type AS category, institution,
           ...
           DATE_FORMAT(remind_date,'%Y-%m-%d')  AS remind_date,
           inactive
    FROM operational_permits WHERE id=?`, [req.params.id]);
...
router.post('/', async (req,res)=>{
  try{
    const { doc_name, category, institution, start_date, expired_date, inactive } = req.body;
    ...
    const [result] = await pool.query(`
      INSERT INTO operational_permits
      (doc_name, type, institution, start_date, expired_date, remind_date, inactive)
      VALUES (?,?,?,?,?,?,?)`,
      [doc_name||'', category||'', institution||'', start, exp, remind_date, inact]
    );
    const [rows] = await pool.query(`
      SELECT id, doc_name, type AS category, institution,
             DATE_FORMAT(start_date,'%Y-%m-%d')   AS start_date,
             ...
             DATE_FORMAT(remind_date,'%Y-%m-%d')  AS remind_date,
             inactive
      FROM operational_permits WHERE id=?`, [result.insertId]);
    const row = rows[0];
    res.status(201).json({ ...row, status: calcStatus(row) });
  }catch(e){ console.error(e); res.status(500).json({error:'Failed to create record'}); }
});
...
router.put('/:id', async (req,res)=>{
  try{
    ...
    const inact = toBool01(inactive);
    await pool.query(`
      UPDATE operational_permits
      SET doc_name=?, type=?, institution=?, start_date=?, expired_date=?, remind_date=?, inactive=?
      WHERE id=?`,
      [doc_name||'', category||'', institution||'', start, exp, remind_date, inact, req.params.id]
    );
    const [rows] = await pool.query(`
      SELECT id, doc_name, type AS category, institution,
             DATE_FORMAT(start_date,'%Y-%m-%d')   AS start_date,
    ...
    const row = rows[0];
    res.json({ ...row, status: calcStatus(row) });
  }catch(e){ console.error(e); res.status(500).json({error:'Failed to update record'}); }
});

router.post('/import/preview', csvUpload.single('file'), async (req,res)=>{
  try{
    if (!req.file || !req.file.buffer) return res.status(400).json({ error:"CSV file is required (field name: 'file')" });
    ...
    const records = parse(raw, { columns:true, skip_empty_lines:true, relax_column_count:true, trim:true, delimiter, bom:true });
    ...
    const [existing] = await pool.query(`
      SELECT
        ...
        DATE_FORMAT(expired_date,'%Y-%m-%d') AS expired_date
      FROM operational_permits
    `);
    const existingKeySet = new Set(existing.map(r => [r.doc_name||'', r.category||'', r.institution||'', r.expired_date||''].join('|')));
    ...
    for (const row of records) {
      ...
      const doc_name    = L['dokumen']   ?? L['document'] ?? row.doc_name ?? row.doc ?? row.document ?? '';
      ...
      const start_raw   = L['tanggal berlaku'] ?? L['start date'] ?? row.start_date ?? null;
      const exp_raw     = L['tanggal kedaluwarsa'] ?? L['tanggal kedaluarsa'] ?? L['expired date'] ?? row.expired_date ?? null;
      const remind_raw  = L['tanggal pengingat'] ?? L['tanggal pengingat (−2 bln)'] ?? L['remind date'] ?? row.remind_date ?? null;
      ...
      const normalized = { doc_name, category, institution, start_date:start, expired_date:exp, remind_date, inactive };
      ...
      preview.push({ key, ...normalized, status: calcStatus({ ...normalized }), dup_in_file, dup_in_db });
    }
    ...
    try{
      ...
      for (const r of rows) {
        ...
        const [exists] = await conn.query(
          `SELECT id FROM operational_permits
           WHERE LOWER(TRIM(doc_name))=? AND LOWER(TRIM(type))=? AND LOWER(TRIM(institution))=? AND DATE_FORMAT(expired_date,'%Y-%m-%d')=? LIMIT 1`,
          [doc_name.toLowerCase().trim(), category.toLowerCase().trim(), institution.toLowerCase().trim(), exp || '']
        );
        ...
        await conn.query(`
          ...
          (doc_name, type, institution, start_date, expired_date, remind_date, inactive)
          VALUES (?,?,?,?,?,?,?)`,
          [doc_name, category, institution, start, exp, remind_date, inactive]
        );
        imported++;
      }
      await conn.commit();
...
router.delete('/:id', async (req, res) => {
  try {
    await pool.query('DELETE FROM operational_permits WHERE id=?', [req.params.id]);
    res.json({ ok: true });
  } catch (e) {
    console.error('Delete operational error:', e);
